...
async function getOsrmFootDistance(lat1, lon1, lat2, lon2) {
...
async function getOsrmTable(source, destinations) {
...
class Raptor {
    ...
    async findRoute(startLat, startLon, destLat, destLon, departureTimeSeconds, maxTransfers = 3, allowedRouteGroups = [1, 2, 3], maxWalkDist = 500) {
        const K = maxTransfers + 1;
        const tau = Array.from({ length: K + 1 }, () => new Map());
        const tau_star = new Map();
        const pointers = new Map();
        const getTau = (k, p) => tau[k].has(p) ? tau[k].get(p) : Infinity;
        const getTauStar = (p) => tau_star.has(p) ? tau_star.get(p) : Infinity;
        const setTau = (k, p, time) => tau[k].set(p, time);
        const setTauStar = (p, time) => tau_star.set(p, time);
        let markedStops = new Set();
        const startRadius = maxWalkDist;
        const destRadius = maxWalkDist;
        const walkingSpeed = 1.2;
        let sourceStops = Array.from(this.data.stops.values())
        ...
        let destStops = Array.from(this.data.stops.values())
        ...
        if (sourceStops.length > 0) {
            const osrmSources = await getOsrmTable({ lat: startLat, lon: startLon }, sourceStops.map(s => ({ lat: s.stop.lat, lon: s.stop.lon })));
            if (osrmSources) {
        ...
        sourceStops = validSourceStops;
        const validDestStops = [];
        if (destStops.length > 0) {
            const osrmDests = await getOsrmTable({ lat: destLat, lon: destLon }, destStops.map(s => ({ lat: s.stop.lat, lon: s.stop.lon })));
            if (osrmDests) {
        ...
        for (let k = 1; k <= K; k++) {
                ...
                for (let i = startIndex; i < pattern.stops.length; i++) {
                    ...
                    if (t !== null && t.stopTimes[i] && boardTime !== null && boardStopId !== null) {
                        ...
                        if (destStopIds.has(pi)) {
                            ...
                            if (dest)
                                destWalkTime = Math.ceil(dest.dist / walkingSpeed);
                        }
            ...
            const tau_k_trips = new Map();
            const trip_pointers = new Map();
            ...
            for (const p of stopsToProcess) {
                ...
                for (const tr of transfers) {
                    if (maxWalkDist !== -1 && tr.distanceMeter > maxWalkDist)
                        continue;
        ...
        const reconstructJourneyForK = (destStopId, finalK, finalTime) => {
            const journey = [];
